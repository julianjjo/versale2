...
export async function collectMetrics(session: import("@playwright/test").CDPSession) {
  try {
...
export async function attachCdpAudit(page: Page) {
  ...
  // ponytail: single CDPSession per page
  ...
  try { await session.send("Performance.enable"); } catch {}

  session.on("Runtime.exceptionThrown", (p: { exceptionDetails: { text: string; exception?: { description?: string } } }) => {
    const d = p.exceptionDetails;
    const msg = d.exception?.description || d.text || "exception";
    consoleErrors.push(msg);
  });
  session.on("Log.entryAdded", (p: { entry: { level: string; text: string } }) => {
    if (p.entry.level === "error") consoleErrors.push(p.entry.text);
  });
  session.on("Network.responseReceived", (p: { response: { url: string; status: number } }) => {
    ...
    await page.evaluate(() => {
      try {
        ...
        const obs = new PerformanceObserver((list: PerformanceObserverEntryList) => {
          // @ts-ignore
          window.__qaLongTasks = ((window as unknown as { __qaLongTasks?: number }).__qaLongTasks || 0) + list.getEntries().length;
        });
  ...
  function getResult(): AuditResult {
    ...
    return { consoleErrors, hydrationErrors, failedRequests, duplicateRequests, metrics: {}, longTasks };
  }
  ...
  async function getResultWithMetrics(): Promise<AuditResult> {
    ...
    try {
      const v = await page.evaluate(() => (window as unknown as { __qaLongTasks?: number }).__qaLongTasks || 0);
      base.longTasks = v;
    } catch { base.longTasks = longTasks; }
    return base;
  }
  ...
  return { session, getResult, getResultWithMetrics, collectMetrics: () => collectMetrics(session), detach };
}
