...
// ponytail: serial — backdate mutates shared e2e.db; upgrade: per-test DB isolation or per-worker e2e.db if parallel needed
...
async function createSellerProduct(
  ...
  const r = await req.post(`${API_URL}/products`, {
    ...
    data: {
      title,
      description: "Prenda e2e borrado.",
      category: "Chaquetas",
      size: "M",
      condition: "Good",
      price: 50000,
  ...
  if (!r.ok())
    throw new Error(`create product ${title}: ${r.status()} ${await r.text()}`);
  ...
  if (!ar.ok())
    throw new Error(`approve ${p.id}: ${ar.status()} ${await ar.text()}`);
...
function prismaForE2e() {
  ...
  const url =
    ...
    `file:${path.join(__dirname, "../../apps/api/e2e.db")}`;
...
test.describe("Borrado de cuenta con anonimización", () => {
  test("1 DangerZone UI happy delete + API variant", async ({ page }) => {
    ...
    const fav = await req.post(`${API_URL}/favorites/${product.id}`, {
      headers: hdr(buyer.token),
    });
    expect([201, 200].includes(fav.status())).toBe(true);
    const cart = await req.post(`${API_URL}/cart/items`, {
      headers: hdr(buyer.token),
    ...
    await expect(
      page.getByText("¿Seguro que quieres eliminar tu cuenta?"),
    ).toBeVisible();
    await page
      .getByRole("button", { name: /Sí, eliminar definitivamente/i })
      .click();
    await expect(
      page.getByText(/La contraseña actual es incorrecta/i),
    ...
    await page
      .getByRole("button", { name: /Sí, eliminar definitivamente/i })
      .click();
    await page.waitForURL(/\/login\?reason=account_deleted/, {
      timeout: 10_000,
    });
    ...
    await expect(
      page.getByText(/Tu cuenta se eliminó correctamente/i),
    ).toBeVisible();

    // API variant: fresh buyer delete via API directly 200
    const buyer2 = await createBuyer(req);
    const del = await req.delete(`${API_URL}/users/me`, {
      headers: hdr(buyer2.token),
      data: { currentPassword: buyer2.password },
    });
  ...
  test("2 token invalidation + forgot enumeration guard", async ({ page }) => {
    ...
    const del = await req.delete(`${API_URL}/users/me`, {
      headers: hdr(oldToken),
      data: { currentPassword: pwd },
    });
    expect(del.status()).toBe(200);
    ...
    const relogin = await req.post(`${API_URL}/auth/login`, {
      data: { email, password: pwd },
    });
    expect(relogin.status()).toBe(401);
    expect(await relogin.text()).toMatch(/Credenciales inválidas/i);

    const forgot = await req.post(`${API_URL}/auth/forgot-password`, {
      data: { email },
    });
  ...
  test("3 email liberado para re-registro 201", async ({ page }) => {
    ...
    const signup = await req.post(`${API_URL}/auth/signup`, {
      data: { email, name: "Re-registro", password: pwd, acceptedTerms: true },
    });
    expect(signup.status()).toBe(201);
    const body = await signup.json();
    expect(body.access_token).toBeTruthy();

    const newLogin = await req.post(`${API_URL}/auth/login`, {
      data: { email, password: pwd },
    });
  ...
  test("4 producto WITHDRAWN tras borrado", async ({ page }) => {
    ...
    const del = await req.delete(`${API_URL}/users/me`, {
      ...
      data: { currentPassword: seller.password },
    });
    expect(del.status()).toBe(200);
    ...
    const mine = await req.get(`${API_URL}/products/mine`, {
      headers: hdr(seller.token),
    });
    ...
    const adminToken = await login(req, E2E_USERS.admin);
    const adminView = await req.get(`${API_URL}/products/${product.id}`, {
      headers: hdr(adminToken),
    });
  ...
  test("5 cascada deletes: cart/favorite/vote/notification", async ({
    page,
  }) => {
    const req = page.request;
    const seller = await createBuyer(req);
    ...
    // favorite + cart
    const f = await req.post(`${API_URL}/favorites/${product.id}`, {
      headers: hdr(buyer.token),
    });
    expect([200, 201].includes(f.status())).toBe(true);
    const c = await req.post(`${API_URL}/cart/items`, {
      headers: hdr(buyer.token),
      data: { productId: product.id, quantity: 1 },
    });
    ...
    const me = await req.get(`${API_URL}/users/me`, {
      headers: hdr(buyer.token),
    });
    ...
    try {
      const favCountBefore = await prisma.favorite.count({
    ...
    const del = await req.delete(`${API_URL}/users/me`, {
      headers: hdr(buyer.token),
      data: { currentPassword: buyer.password },
    });
    expect(del.status()).toBe(200);

    // old token 401 for favorites
    const favAfter = await req.get(`${API_URL}/favorites`, {
      headers: hdr(buyer.token),
    });
    ...
    try {
      const favCount = await prisma2.favorite.count({
        where: { userId: buyerId },
      });
      expect(favCount).toBe(0);
      const cartAfter = await prisma2.cart.findUnique({
        where: { userId: buyerId },
      });
      expect(cartAfter).toBeNull();
      const voteCount = await prisma2.reviewHelpfulVote.count({
        where: { userId: buyerId },
      });
      expect(voteCount).toBe(0);
      const notifCount = await prisma2.notification.count({
        where: { userId: buyerId },
      });
  ...
  test("6 redacción dirección 30d — ponytail ceiling", async ({ page }) => {
    ...
    const product = await createSellerProduct(req, seller.token, title);
    await req.post(`${API_URL}/cart/items`, {
      headers: hdr(buyer.token),
      data: { productId: product.id, quantity: 1 },
    });
    const orderRes = await req.post(`${API_URL}/orders`, {
      headers: hdr(buyer.token),
    ...
    const ship = await req.patch(
      `${API_URL}/orders/mine/sales/${order.id}/ship`,
      { headers: hdr(seller.token), data: { trackingNumber: "REDACT-TRACK" } },
    );
    expect(ship.status()).toBe(200);
    const del = await req.patch(`${API_URL}/orders/admin/${order.id}/status`, {
      headers: hdr(adminToken),
    ...
    const beforeDel = await req.get(`${API_URL}/orders/${order.id}`, {
      headers: hdr(buyer.token),
    });
    expect(beforeDel.status()).toBe(200);
    expect((await beforeDel.json()).shippingAddress).toMatchObject(
      E2E_SHIPPING_ADDRESS,
    );

    // delete buyer
    const delBuyer = await req.delete(`${API_URL}/users/me`, {
      headers: hdr(buyer.token),
      data: { currentPassword: buyer.password },
    });
    ...
    const asAdmin = await req.get(`${API_URL}/orders/${order.id}`, {
      headers: hdr(adminToken),
    });
    ...
    try {
      ...
      await prisma.order.updateMany({
        where: {
          ...
          deliveredAt: { lte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
        },
        data: {
          shippingAddress: {
            eliminada:
    ...
    const after = await req.get(`${API_URL}/orders/${order.id}`, {
      headers: hdr(adminToken),
    });
    expect(after.status()).toBe(200);
    const redacted = (await after.json()).shippingAddress;
    expect(redacted.eliminada ?? JSON.stringify(redacted)).toMatch(
      /eliminada/i,
    );
  });
  ...
  test("7 last-admin guard y self guard", async ({ page }) => {
    ...
    const self = await req.delete(`${API_URL}/users/${adminId}`, {
      headers: hdr(adminToken),
    });
    ...
    const secondToken = await login(req, {
      email: second.email,
      password: second.password,
    });

    // delete second admin when count 2 → ok
    const delSecond = await req.delete(`${API_URL}/users/${secondId}`, {
      headers: hdr(adminToken),
    });
    expect(delSecond.status()).toBe(200);

    // now only one admin left → deleteOwnAccount last-admin 403
    const last = await req.delete(`${API_URL}/users/me`, {
      headers: hdr(adminToken),
      data: { currentPassword: E2E_USERS.admin.password },
    });
    expect(last.status()).toBe(403);
    expect(await last.text()).toMatch(/último administrador/i);

    // verify still active
    const still = await req.get(`${API_URL}/users/me`, {
      headers: hdr(adminToken),
    });
    ...
    const secondMe = await req.get(`${API_URL}/users/me`, {
      headers: hdr(secondToken),
    });
  ...
  test("8 wrong password guard 403", async ({ page }) => {
    const req = page.request;
    const buyer = await createBuyer(req);
    const bad = await req.delete(`${API_URL}/users/me`, {
      headers: hdr(buyer.token),
      data: { currentPassword: "incorrecta123" },
    });
