...
test.describe("Flujos de cuenta: verificación y recuperación", () => {
  test("happy verify: signup → verify → isVerified true → reuse 400", async ({
    page,
  }) => {
    ...
    const vr = await req.post(`${API_URL}/auth/verify-email`, {
      data: { token },
    });
    ...
    const reuse = await req.post(`${API_URL}/auth/verify-email`, {
      data: { token },
    });
  ...
  test("verify expirado: token con TTL vencido → 400", async ({ page }) => {
    ...
    const { verificationToken: tok } = (await s.json()) as {
  ...
  test("resend: reenvía, invalida anterior, bloquea verificado y sin auth 401", async ({
    page,
  }) => {
    const req = page.request;
    const email = uniqueEmail();
    ...
    expect(s.status()).toBe(201);
    const { verificationToken: oldTok, access_token: at } =
      (await s.json()) as {
        verificationToken: string;
        access_token: string;
      };

    const rs = await req.post(`${API_URL}/auth/resend-verification`, {
      headers: hdr(at),
      data: {},
    });
    ...
    try {
      await prisma.user.update({
        where: { email },
        data: {
          verificationToken: hashToken(known),
    ...
    const vr2 = await req.post(`${API_URL}/auth/verify-email`, {
      data: { token: known },
    });
    expect(vr2.status()).toBe(200);

    const rs2 = await req.post(`${API_URL}/auth/resend-verification`, {
      headers: hdr(at),
      data: {},
    });
    ...
    const noAuth = await req.post(`${API_URL}/auth/resend-verification`, {
      data: {},
    });
    expect(noAuth.status()).toBe(401);
  });

  test("forgot → reset happy: actualiza pwd, invalida antiguo y single-use", async ({
    page,
  }) => {
    const req = page.request;
    const email = uniqueEmail();
    ...
    const fg = await req.post(`${API_URL}/auth/forgot-password`, {
      data: { email },
    });
    ...
    const rs = await req.post(`${API_URL}/auth/reset-password`, {
      data: { token: resetToken, password: newPwd },
    });
    expect(rs.status()).toBe(200);
    expect((await rs.json()).message).toMatch(/se actualizó/i);

    const oldLogin = await req.post(`${API_URL}/auth/login`, {
      data: { email, password: oldPwd },
    });
    ...
    const reuse = await req.post(`${API_URL}/auth/reset-password`, {
      data: { token: resetToken, password: "otra12345!" },
    });
    expect(reuse.status()).toBe(400);
  ...
  test("reset expirado y token inválido → 400", async ({ page }) => {
    ...
    const r2 = await req.post(`${API_URL}/auth/reset-password`, {
      data: {
        token: "garbage-not-a-token-" + Date.now(),
  ...
  test("forgot enumeration guard: correo inexistente no fuga token", async ({
    page,
  }) => {
    const req = page.request;
    const ghost = `ghost-${Date.now()}-${Math.random().toString(36).slice(2, 6)}@e2e.test`;
    const r = await req.post(`${API_URL}/auth/forgot-password`, {
      data: { email: ghost },
    });
  ...
  test("UI: verify-email", async ({ page }) => {
    ...
    const { verificationToken } = (await s.json()) as {
      verificationToken: string;
    };
    await page.goto(
  ...
  test("UI: reset-password", async ({ page }) => {
    ...
    await expect(page.getByText(/se actualiz/i)).toBeVisible({
      timeout: 10_000,
    });
    const l = await req.post(`${API_URL}/auth/login`, {
      data: { email, password: "uiNueva123!" },
    });
