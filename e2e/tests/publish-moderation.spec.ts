...
async function login(
  ...
  if (!r.ok())
    throw new Error(`login ${c.email}: ${r.status()} ${await r.text()}`);
  return (await r.json()).access_token as string;
}
async function createProductAs(
  req: import("@playwright/test").APIRequestContext,
  token: string,
  o: Partial<{ title: string; category: string; price: number }> = {},
) {
  const title =
    o.title ?? `PubMod ${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
  const r = await req.post(`${API_URL}/products`, {
    headers: hdr(token),
    data: {
      title,
      description: "Prenda publish-moderation e2e.",
      category: o.category ?? "Chaquetas",
      size: "M",
      condition: "Good",
      price: o.price ?? 50000,
    },
  ...
  if (!r.ok())
    throw new Error(`create ${title}: ${r.status()} ${await r.text()}`);
  const p = await r.json();
  return { id: p.id as string, title };
}
async function isVisibleInCatalog(
  req: import("@playwright/test").APIRequestContext,
  title: string,
) {
  const r = await req.get(
    `${API_URL}/products?search=${encodeURIComponent(title)}`,
  );
  if (!r.ok())
    throw new Error(`catalog search ${title}: ${r.status()} ${await r.text()}`);
  const j = await r.json();
  return (j.data as Array<{ title: string }>).some((x) => x.title === title);
}

test.describe("Publicación y moderación P1 — publish-moderation", () => {
  test("a) PENDING invisible en catálogo, visible en /mine y /admin/all", async ({
    request,
  }) => {
    const seller = await createBuyer(request);
    const prod = await createProductAs(request, seller.token, {
      title: `PEND ${Date.now()}-${Math.random().toString(36).slice(2, 4)}`,
    });
    ...
    const mineData = (await mine.json()).data as Array<{
      id: string;
      isApproved: boolean;
    ...
    const adminAll = await request.get(
      `${API_URL}/products/admin/all?status=pending`,
      { headers: hdr(adminToken) },
    );
    expect(adminAll.status()).toBe(200);
    const allIds = ((await adminAll.json()).data as Array<{ id: string }>).map(
      (x) => x.id,
    );
  ...
  test("b) admin approve → aparece en catálogo", async ({ request }) => {
    ...
    const prod = await createProductAs(request, seller.token);
    const adminToken = await login(request, E2E_USERS.admin);
    const ar = await request.patch(
      `${API_URL}/products/admin/${prod.id}/approve`,
      { headers: hdr(adminToken), data: {} },
    );
    expect(ar.status()).toBe(200);
    expect((await ar.json()).isApproved).toBe(true);
    expect(await isVisibleInCatalog(request, prod.title)).toBe(true);
    const mine = await request.get(`${API_URL}/products/mine?status=approved`, {
      headers: hdr(seller.token),
    });
    expect(
      ((await mine.json()).data as Array<{ id: string }>).map((x) => x.id),
    ).toContain(prod.id);
  });

  test("c) admin reject con reason, sin reason 400, re-reject idempotente", async ({
    request,
  }) => {
    const seller = await createBuyer(request);
    const prod = await createProductAs(request, seller.token);
    ...
    const rr = await request.patch(
      `${API_URL}/products/admin/${prod.id}/reject`,
      { headers: hdr(adminToken), data: { reason } },
    );
    ...
    expect(await isVisibleInCatalog(request, prod.title)).toBe(false);
    // mine rejected
    const mine = await request.get(`${API_URL}/products/mine?status=rejected`, {
      headers: hdr(seller.token),
    });
    const mineR = (
      (await mine.json()).data as Array<{ id: string; rejectionReason: string }>
    ).find((x) => x.id === prod.id);
    expect(mineR?.rejectionReason).toBe(reason);
    const prod2 = await createProductAs(request, seller.token);
    const noReason = await request.patch(
      `${API_URL}/products/admin/${prod2.id}/reject`,
      { headers: hdr(adminToken), data: {} },
    );
    expect(noReason.status()).toBe(400);
    expect(await noReason.text()).toMatch(/motivo.*obligatorio/i);
    // re-reject ya rechazado → bulkReject excluye where rejectedAt null; single reject reescribe (200) — acepta 200/400
    const dup = await request.patch(
      `${API_URL}/products/admin/${prod.id}/reject`,
      { headers: hdr(adminToken), data: { reason: "otro" } },
    );
  ...
  test("d) bulk approve y bulk reject", async ({ request }) => {
    ...
    const bulkA = await request.patch(
      `${API_URL}/products/admin/bulk-approve`,
      { headers: hdr(adminToken), data: { ids: [p1.id, p2.id] } },
    );
    ...
    const bulkR = await request.patch(`${API_URL}/products/admin/bulk-reject`, {
      headers: hdr(adminToken),
      data: { ids: [p3.id, p4.id], reason: "Incumple políticas" },
    });
  ...
  test("e) pause/unpause single + bulk y 403 en ajeno", async ({ request }) => {
    ...
    await request.patch(`${API_URL}/products/admin/${p.id}/approve`, {
      headers: hdr(adminToken),
      data: {},
    });
    const pause = await request.patch(`${API_URL}/products/${p.id}/pause`, {
      headers: hdr(seller.token),
      data: {},
    });
    expect(pause.status()).toBe(200);
    expect(await isVisibleInCatalog(request, p.title)).toBe(false);
    const minePaused = await request.get(
      `${API_URL}/products/mine?status=paused`,
      { headers: hdr(seller.token) },
    );
    expect(
      ((await minePaused.json()).data as Array<{ id: string }>).map(
        (x) => x.id,
      ),
    ).toContain(p.id);
    const unpause = await request.patch(`${API_URL}/products/${p.id}/unpause`, {
      headers: hdr(seller.token),
      data: {},
    });
    ...
    await request.patch(`${API_URL}/products/admin/bulk-approve`, {
      headers: hdr(adminToken),
      data: { ids: [b1.id, b2.id] },
    });
    const bp = await request.patch(`${API_URL}/products/bulk-pause`, {
      headers: hdr(seller.token),
      data: { ids: [b1.id, b2.id] },
    });
    ...
    const bu = await request.patch(`${API_URL}/products/bulk-unpause`, {
      headers: hdr(seller.token),
      data: { ids: [b1.id, b2.id] },
    });
    ...
    const forbidden = await request.patch(
      `${API_URL}/products/${b1.id}/pause`,
      { headers: hdr(other.token), data: {} },
    );
    expect(forbidden.status()).toBe(403);
  });

  test("f) edit guards: propio pending ok, ajeno 403, SOLD bloqueado", async ({
    request,
  }) => {
    const seller = await createBuyer(request);
    const other = await createBuyer(request);
    const prod = await createProductAs(request, seller.token);
    const editOk = await request.patch(`${API_URL}/products/${prod.id}`, {
      headers: hdr(seller.token),
      data: { title: prod.title + " edit" },
    });
    expect(editOk.status()).toBe(200);
    expect((await editOk.json()).title).toContain("edit");
    const editForbid = await request.patch(`${API_URL}/products/${prod.id}`, {
    ...
    const sProd = await createProductAs(request, seller2.token, {
      title: `SOLD ${Date.now()}-${Math.random().toString(36).slice(2, 4)}`,
    });
    const adminToken = await login(request, E2E_USERS.admin);
    await request.patch(`${API_URL}/products/admin/${sProd.id}/approve`, {
      headers: hdr(adminToken),
      data: {},
    });
    const buyer = await createBuyer(request);
    const add = await request.post(`${API_URL}/cart/items`, {
      headers: hdr(buyer.token),
      data: { productId: sProd.id, quantity: 1 },
    });
    expect(add.status()).toBe(201);
    const ord = await request.post(`${API_URL}/orders`, {
      headers: hdr(buyer.token),
      data: { shippingAddress: E2E_SHIPPING_ADDRESS },
    });
    expect(ord.status()).toBe(201);
    const editSold = await request.patch(`${API_URL}/products/${sProd.id}`, {
      headers: hdr(seller2.token),
      data: { title: "intento edit sold" },
    });
    expect([400, 403].includes(editSold.status())).toBe(true);
  });

  test("g) límite 20 activas → 21st 429, tras liberar 21st ok", async ({
    request,
  }) => {
    const seller = await createBuyer(request);
    const ids: string[] = [];
    for (let i = 0; i < 20; i++) {
      const p = await createProductAs(request, seller.token, {
        title: `LIM ${Date.now()}-${i}-${Math.random().toString(36).slice(2, 4)}`,
        price: 10000 + i,
      });
      ids.push(p.id);
    }
    const p21 = await request.post(`${API_URL}/products`, {
      headers: hdr(seller.token),
      data: {
        title: `LIM21 ${Date.now()}`,
        description: "x",
        category: "Chaquetas",
        size: "M",
        condition: "Good",
    ...
    const afterDel = await request.post(`${API_URL}/products`, {
      headers: hdr(seller.token),
      data: {
        title: `LIM21b ${Date.now()}`,
        description: "x",
        category: "Chaquetas",
        size: "M",
        condition: "Good",
  ...
  test("h) filtro categoría y DTO validación", async ({ request }) => {
    ...
    const j1 = await createProductAs(request, seller.token, {
      title: `Cat Cha ${uniq}`,
      category: "Chaquetas",
    });
    const j2 = await createProductAs(request, seller.token, {
      title: `Cat Jean ${uniq}`,
      category: "Jeans",
    });
    await request.patch(`${API_URL}/products/admin/bulk-approve`, {
      headers: hdr(adminToken),
      data: { ids: [j1.id, j2.id] },
    });
    ...
    const badGet = await request.get(
      `${API_URL}/products?category=NoExisteXYZ`,
    );
    expect([200, 400].includes(badGet.status())).toBe(true);
    // POST categoría inválida debe 400
    const badPost = await request.post(`${API_URL}/products`, {
      headers: hdr(seller.token),
      data: {
        title: `BadCat ${uniq}`,
        description: "x",
        category: "InvalidaXYZ",
        size: "M",
        condition: "Good",
        price: 50000,
      },
