...
```javascript
test('calculates total', () => {
  expect(0.1 + 0.2).toBeCloseTo(0.3); // PASSES — passes under the default precision (2)
});
...
`toBeCloseTo(expected, precision)` checks that `|received - expected| < 10^(-precision) / 2`. The default precision is `2`, meaning differences smaller than `0.005` are considered equal.

| Precision | Tolerance | Use case |
|---|---|---|
| 0 | 0.5 | Rough estimates |
| 2 | 0.005 | Currency (2 decimal places) — default |
| 5 | 0.000005 | General floating-point (tighter than default) |
