...
// this component — it needs `useSearchParams`/`usePathname` (for pagination)
// on top of `useParams` (the seller id from the URL segment). `push`/`replace`
...
// the component must filter the catalog by the URL's id (available
// immediately, with no fetch required), not by echoing this response's `id`
// field, so a mismatch here would expose that bug immediately instead of
// masking it.
const mockProfile = {
...
describe("SellerProfileContent", () => {
  ...
  // The server-rendered page seeds this via `initialProfile` (see
  // app/vendedores/[id]/page.tsx) — this proves the seed alone is enough to
  // paint the header with no loading spinner and no extra request, before
  // the client's own refetch (if any) settles.
  it("pinta de inmediato con el perfil sembrado por el servidor, sin esperar un fetch", () => {
    vi.mocked(api.get).mockImplementation(async (url: string) => {
      if (url === "/products") return { data: mockProducts };
      return { data: {} };
    });
    ...
    render(
      <TestProviders>
        <SellerProfileContent initialProfile={mockProfile} />
      </TestProviders>,
    );
  ...
  // A real 404 is caught server-side now (app/vendedores/[id]/page.tsx calls
  // notFound() before this component ever mounts) — this covers the narrow
  // race where the seller existed for the server's probe but is gone by the
  // time the client's own refetch runs, so this branch still degrades
  // gracefully instead of crashing.
  it("muestra 'vendedor no encontrado' si la propia recarga del cliente responde 404", async () => {
