...
- `e2e/tests/account-flows.spec.ts:21`, cron not exposed via HTTP — direct DB backdate is minimal e2e bridge. ceiling: cron not exposed. upgrade: endpoint exists `POST /auth/debug/backdate` (test only, #413) — `verify expirado` + `reset expirado` tests now use HTTP (migrated 2026-08-28), helper remains for other backdates.
